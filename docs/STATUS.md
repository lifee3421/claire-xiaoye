...
- 复盘追踪器管理已支持 Schema 字段分组、多选展示指标、日/周/月/年周期、天/周/月/年间隔、日期范围、截止日、提前提醒、暂停、删除与恢复默认；事实仍只读取最终结算。
- 设置页新增动态复盘项目：可新增、改名、调整顺序、暂停、归档和删除；未暂停/归档项目会代入默认 Markdown 模板，profile 在 Firebase 与 demo 模式均持久化 `reviewProjects`。
- 周大表三级分类自动聚合、分类的全层级拖拽/归档交互、真实浏览器与 Firebase 登录回归仍未完成。
...
## 12. 统一卡片、生活分类与多日排程（2026-07-20）

- 用户可见排程已统一为普通卡片：内置晨间、午餐、午休、晚餐和睡前内容与旧 `fixedEvents` 读取结果都走任务卡片交互；保存时旧事件迁移到 `todayCustomBlocks` 并清空旧数组，仍保留旧格式读取兼容。
- 分类树会补齐稳定的生活二级分类 ID；晨间锚点与跨仓库午餐协议分别使用 `life.morning-routine`、`life.lunch`，不依赖可编辑中文标题。`AgentDaySnapshot` 的 timeline/currentByClock/nextTask 均保留 `categoryId`。
- 复盘追踪管理可选择生活二级分类，并按卡片完成次数、完成天数和完成率统计，不计入学习分钟。
- Today/Tomorrow/任意日期继续使用日期隔离草稿；新增 1–7 天逐日预排，自定义共享任务按稳定 `sourceTaskId` 只分配一次。
- 拖拽 overlay 不再在进入时间线时消失，任务池原卡仅保留透明占位，时间线与任务池互为 drop target，启用边缘自动滚动；矮卡完成点击区域为 40×40px。
- 2026-07-20 验证：`node --test` 102/102；真实路径 `npm.cmd run build` 成功。demo 页面确认旧固定事件入口为 0、六张生活卡均为普通可完成卡、生活追踪可选择 `life.lunch`、未来日期入口正常；浏览器自动化的鼠标拖放未能可靠触发，因此实际手势轨迹仍建议部署后用真实鼠标复核。
