 ...
 * download URL.  The image is stored under users/{uid}/dashboard/goal-image
 * (a single fixed object per user, with no file extension) so it is
 * naturally scoped per-user.  The MIME type is carried via Storage
 * metadata contentType rather than via the file name.
 *
...
export async function uploadGoalImage(uid, blob) {
  ...
  // Single fixed object per user, with no file extension.  The MIME type is
  // carried via Storage metadata contentType (see uploadBytes below) so the
  // same logical path can hold png/jpg/webp without renaming.
  const path = `users/${uid}/dashboard/goal-image`;
