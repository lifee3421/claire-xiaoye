...
import { buildConfirmedAmbiguousMigrationEvent, buildSettlementFieldDiagnostic, buildTrackerMigrationDryRun, resolveMigrationRange } from "../utils/trackerMigration.js";
...
export default function TrackerMigrationPanel({ trackers, migrationState, onLoadSnapshot, onConfirm, onBack }) {
  const [scope, setScope] = useState("current_month"); const [start, setStart] = useState(""); const [end, setEnd] = useState(""); const [snapshot, setSnapshot] = useState(null); const [preview, setPreview] = useState(null); const [diagnostic, setDiagnostic] = useState(null); const [selected, setSelected] = useState(new Set()); const [choices, setChoices] = useState({}); const [error, setError] = useState(""); const [message, setMessage] = useState(""); const [loading, setLoading] = useState(false);
  const range = useMemo(() => resolveMigrationRange({ scope, today: todayIso(), start, end }), [scope, start, end]);
  const scan = async () => { setLoading(true); setError(""); setMessage(""); try { const nextSnapshot = await onLoadSnapshot(); const nextPreview = await buildTrackerMigrationDryRun({ trackers, settlements: nextSnapshot.settlements, existingEvents: nextSnapshot.events, range }); setSnapshot(nextSnapshot); setPreview(nextPreview); setDiagnostic(buildSettlementFieldDiagnostic(nextSnapshot.settlements)); setSelected(new Set(nextPreview.highConfidence.map((candidate) => candidate.id))); } catch (scanError) { setError(scanError instanceof Error ? scanError.message : String(scanError)); } finally { setLoading(false); } };
  ...
  return <div className="tracker-migration-panel"><div className="manager-fixed-head"><div><h3>历史记录迁移</h3><p>预览不会修改历史复盘。只扫描已保存 settlements，不读取时间轴、Focus、贴纸或草稿。</p></div><button className="secondary-button compact" type="button" onClick={onBack}>返回追踪项</button></div><div className="tracker-manager-scroll"><section className="tracker-edit-section"><div className="two-column-fields"><label className="field"><span>扫描范围</span><select value={scope} onChange={(event) => setScope(event.target.value)}><option value="current_month">当前月</option><option value="previous_month">上个月</option><option value="custom">自定义日期范围</option><option value="all">全部历史</option></select></label>{scope === "custom" && <><label className="field"><span>开始日期</span><input type="date" value={start} onChange={(event) => setStart(event.target.value)} /></label><label className="field"><span>结束日期</span><input type="date" value={end} onChange={(event) => setEnd(event.target.value)} /></label></>}<button className="secondary-button compact" type="button" disabled={loading || scope === "custom" && (!start || !end)} onClick={scan}>重新预览</button></div><p className="field-help">迁移状态：{migrationState?.status || "never_run"}；本次范围：{range.scope === "all" ? "全部历史" : `${range.start} 至 ${range.end}`}</p></section>{error && <p className="field-help" role="alert">迁移失败：{error}</p>}{message && <p className="field-help" role="status">{message}</p>}{preview && <><section className="tracker-edit-section"><h4>扫描结果</h4><p>扫描 settlements：{preview.scannedSettlements}；高置信候选：{preview.highConfidence.length}；已存在：{preview.existing.length}；待确认：{preview.ambiguous.length}</p><div className="migration-summary-list">{preview.summaries.map((summary) => <p key={summary.trackerId}><strong>{summary.title}</strong> · 可迁移 {summary.migratable} · 已存在 {summary.existing} · 待确认 {summary.ambiguous} · 无证据日期 {summary.noEvidenceDates}{summary.configurationIssues.length ? ` · 配置问题：${summary.configurationIssues.join("、")}` : ""}</p>)}</div></section>{diagnostic && diagnostic.length > 0 && (() => {
  const catKeys = [...new Set(diagnostic.flatMap((row) => Object.keys(row.categoryDurations)))].sort();
  const fieldKeys = [...new Set(diagnostic.flatMap((row) => Object.keys(row.fieldValues)))].sort();
  const hasMaint = diagnostic.some((row) => row.maintenanceCompleted !== null);
  const hasLegacyEx = diagnostic.some((row) => row.legacyExerciseMinutes !== null);
  const cell = (v) => v == null ? <td>—</td> : <td style={{fontVariantNumeric:"tabular-nums"}}>{String(v)}</td>;
  return (
    <details className="tracker-edit-section" style={{marginBottom:"1rem"}}>
      <summary style={{cursor:"pointer",fontWeight:600}}>字段诊断（{diagnostic.length} 个 settlement）</summary>
      <div style={{overflowX:"auto",marginTop:"0.5rem"}}>
        <table style={{borderCollapse:"collapse",fontSize:"0.75rem",whiteSpace:"nowrap"}}>
          <thead><tr style={{borderBottom:"1px solid var(--border)"}}>
            ...
            {catKeys.map((k) => <th key={k} style={{padding:"2px 6px",textAlign:"right",maxWidth:120,overflow:"hidden",textOverflow:"ellipsis"}} title={k}>cat:{k.length>20?k.slice(-20):k}</th>)}
            {fieldKeys.map((k) => <th key={k} style={{padding:"2px 6px",textAlign:"right",maxWidth:120,overflow:"hidden",textOverflow:"ellipsis"}} title={k}>f:{k.length>22?k.slice(-22):k}</th>)}
            <th style={{padding:"2px 6px"}}>mask</th>
            {hasMaint && <th style={{padding:"2px 6px"}}>maint[]</th>}
            {hasLegacyEx && <th style={{padding:"2px 6px"}}>legacyEx</th>}
          </tr></thead>
          <tbody>
            {diagnostic.map((row) => (
              <tr key={row.date} style={{borderBottom:"1px solid var(--border-subtle,#eee)"}}>
                <td style={{padding:"2px 6px"}}>{row.date}</td>
                {catKeys.map((k) => cell(row.categoryDurations[k]))}
                {fieldKeys.map((k) => cell(row.fieldValues[k]))}
                <td style={{padding:"2px 6px"}}>{row.maskStatus ?? "—"}</td>
                {hasMaint && <td style={{padding:"2px 6px",fontSize:"0.65rem"}}>{row.maintenanceCompleted ? row.maintenanceCompleted.join(",") : "—"}</td>}
                {hasLegacyEx && cell(row.legacyExerciseMinutes)}
              </tr>
            ))}
          </tbody>
        </table>
