...
import { isTrackerConfigured, isValidTrackerTime, resolveTrackerStickerPlacementMode } from "../utils/trackerConfig.js";
import { buildTrackersForProfileSave, createCustomTracker, isDefaultTrackerId, trackerGoalSummary, trackerScheduleSummary, validateTrackerDrafts } from "../utils/trackerManagerModel.js";
import TrackerMonthlyOverview from "./TrackerMonthlyOverview.jsx";
import TrackerMigrationPanel from "./TrackerMigrationPanel.jsx";
import { nextTrackerMigrationState } from "../utils/trackerMigration.js";
...
function ScheduleEditor({ tracker, onChange }) {
  const schedule = tracker.schedule || { kind: "interval", every: 1, unit: "day" }; const update = (patch) => onChange({ schedule: { ...schedule, ...patch } });
  return <section className="tracker-edit-section"><h4>周期</h4><label className="field"><span>周期类型</span><select value={schedule.kind || "interval"} onChange={(event) => onChange({ schedule: event.target.value === "interval" ? { kind: "interval", every: 1, unit: "day" } : event.target.value === "period" ? { kind: "period", period: "week" } : { kind: "deadline", dueDate: "" } })}><option value="interval">间隔</option><option value="period">自然周期</option><option value="deadline">截止日期</option></select></label>{schedule.kind === "interval" && <div className="natural-goal-row">每 <input type="number" min="1" value={schedule.every || ""} onChange={(event) => update({ every: Math.max(1, Number(event.target.value) || 1) })} /> <select value={schedule.unit || "day"} onChange={(event) => update({ unit: event.target.value })}><option value="day">天</option><option value="week">周</option><option value="month">月</option><option value="year">年</option></select></div>}{schedule.kind === "period" && <label className="field"><span>每个周期</span><select value={schedule.period || "week"} onChange={(event) => update({ period: event.target.value })}><option value="week">每周</option><option value="month">每月</option><option value="year">每年</option></select></label>}{schedule.kind === "deadline" && <label className="field"><span>截止日期</span><input type="date" value={schedule.dueDate || ""} onChange={(event) => update({ dueDate: event.target.value })} /></label>}</section>;
}

function GoalEditor({ tracker, onChange }) { const goal = tracker.goal || { aggregation: "occurrence", target: 1, unit: "times" }; const update = (patch) => onChange({ goal: { ...goal, ...patch } }); return <section className="tracker-edit-section"><h4>目标</h4><div className="two-column-fields"><label className="field"><span>目标类型</span><select value={goal.aggregation || "occurrence"} onChange={(event) => update({ aggregation: event.target.value, unit: event.target.value === "active_days" ? "days" : event.target.value === "sum" ? "minutes" : "times" })}><option value="occurrence">完成次数</option><option value="active_days">完成天数</option><option value="sum">累计值</option></select></label><label className="field"><span>目标数值</span><input type="number" min="1" value={goal.target || ""} onChange={(event) => update({ target: Math.max(1, Number(event.target.value) || 1) })} /></label><label className="field"><span>单位</span><input value={goal.unit || ""} onChange={(event) => update({ unit: event.target.value })} /></label></div></section>; }

function StickerEditor({ tracker, onChange }) { const settings = tracker.stickerSettings || { enabled: false, title: "", emoji: "", placementMode: "sticker_bar", time: "", type: "reminder" }; const placementMode = resolveTrackerStickerPlacementMode(settings); const update = (patch) => onChange({ stickerSettings: { ...settings, ...patch } }); const invalid = settings.enabled && placementMode === "timeline" && !isValidTrackerTime(settings.time); return <section className="tracker-edit-section"><h4>自动贴纸</h4><label className="mini-check"><input type="checkbox" checked={settings.enabled === true} onChange={(event) => update({ enabled: event.target.checked })} />启用自动贴纸</label><div className="two-column-fields"><label className="field"><span>贴纸标题</span><input value={settings.title || ""} placeholder={tracker.title || "追踪提醒"} onChange={(event) => update({ title: event.target.value })} /></label><label className="field"><span>贴纸 emoji</span><input value={settings.emoji || ""} placeholder={tracker.emoji || "✨"} onChange={(event) => update({ emoji: event.target.value })} /></label><label className="field"><span>显示位置</span><select value={placementMode} onChange={(event) => update({ placementMode: event.target.value })}><option value="sticker_bar">顶部贴纸栏</option><option value="timeline">时间轴</option></select></label>{placementMode === "timeline" && <label className="field"><span>时间（HH:mm）</span><input type="time" value={settings.time || ""} onChange={(event) => update({ time: event.target.value })} /></label>}</div>{placementMode === "sticker_bar" && <p className="field-help">顶部贴纸栏不需要时间；保留的时间会在切回时间轴时继续使用。</p>}{invalid && <p className="field-help" role="alert">时间轴贴纸必须设置合法 HH:mm，当前不能保存。</p>}<p className="field-help">completion 型贴纸暂未支持；本阶段仅生成 reminder 型贴纸，勾选贴纸不会证明习惯完成。</p></section>; }
...
export default function TrackerManager({ profile = {}, initialOverviewTrackerId = null, onSave, onCancel, onSyncToday, onLoadCompletionEvents, onLoadMigrationSnapshot, onWriteMigrationEvents, hasMigratableHistoryMap = null }) {
  ...
  const overviewTracker = form.find((tracker) => tracker.id === overviewState?.trackerId);
  const confirmMigration = async ({ events, range, selectedAmbiguous }) => {
    let nextForm = form;
    selectedAmbiguous.forEach((candidate) => { const trackerId = candidate.selectedTrackerId; if (!trackerId || !candidate.categoryId) return; const current = nextForm.find((tracker) => tracker.id === trackerId); if (!current?.evidenceBindings?.some((binding) => binding.type === "categoryId" && binding.categoryId === candidate.categoryId)) nextForm = updateById(nextForm, trackerId, { evidenceBindings: [...(current?.evidenceBindings || []), { type: "categoryId", categoryId: candidate.categoryId }] }); });
    if (nextForm !== form) { const trackers = buildTrackersForProfileSave({ initialEffective, editedEffective: nextForm, storedTrackers: profile.trackers }); await onSave({ trackers }); setForm(nextForm); }
    const result = await onWriteMigrationEvents(events);
    const nextMigrationState = nextTrackerMigrationState(migrationState, { range, failed: result.failed }); await onSave({ trackerMigrationState: nextMigrationState }); setMigrationState(nextMigrationState);
