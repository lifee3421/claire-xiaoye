...
function resolveTargetCategoryId(categoryId, level2Index) {
  ...
  // taxonomy snapshot yet. Study target ids are level-2 ids such as
  // study.math / study.english / study.professional, while Focus can carry a
  // more specific leaf such as study.math.calculus.
 ...
 * more specific leaf (for example study.math.calculus); those minutes must
 * still roll up to the visible "数学" target row rather than disappearing.
 *
 * This stays independent of planner-card overlap: a study session still
 * counts when it starts late, runs overtime, or happens outside the original
 * scheduled card window.
 */
