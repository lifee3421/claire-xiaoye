...
// Only ever emitted when there's a real commentary to show — never an
// empty "## ❄️ 雪尘批注" heading with nothing under it.
function snowDustCommentaryMarkdown(draft) {
  const noteText = value(draft, "snowDust.note");
  if (!noteText) return [];
  const generatedAt = draft?.fields?.["snowDust.note"]?.generatedAt;
  const timeText = generatedAt && !Number.isNaN(Date.parse(generatedAt)) ? new Date(generatedAt).toLocaleString("zh-CN") : "";
  return ["", "## ❄️ 雪尘批注", line("批注时间", timeText), line("内容", noteText)];
}
...
export function buildReviewMarkdown(draft, profile = {}, { taxonomy = [], taxonomySnapshot = [] } = {}) {
  ...
  const projects = [...(profile.reviewProjects || []), ...(draft.temporaryProjects || [])]
    ...
    .filter((project, index, list) => list.findIndex((item) => (item.temporaryId || item.id || item.name) === (project.temporaryId || project.id || project.name)) === index);
  const projectMarkdown = projects.flatMap((project) => {
    const schema = reviewSchemaDynamicProject(project);
    const prefix = schema.id.replace(/\.totalMinutes$/, "");
    return ["", `### ${project.name}`, line("总时长", n(schema.id)), line("今日推进", v(`${prefix}.progress`)), line("调整", v(`${prefix}.adjustment`))];
  });
  return [
    ...
    ...snowDustCommentaryMarkdown(draft),
  ].join("\n");
}
