...
export const reviewSections = [
  { title: "学习", groups: [
    { title: "数学", fields: [duration("study.math.totalMinutes", "总时长", { parts: ["study.math.calculus.duration", "study.math.linearAlgebra.duration"] }), duration("study.math.calculus.duration", "高等数学"), duration("study.math.linearAlgebra.duration", "线性代数"), text("study.math.progress", "数学今日推进"), text("study.math.calculus.progress", "高等数学推进"), text("study.math.linearAlgebra.progress", "线性代数推进"), text("study.math.adjustment", "调整"), text("study.math.calculus.adjustment", "高等数学调整"), text("study.math.linearAlgebra.adjustment", "线性代数调整")] },
    { title: "专业课", fields: [duration("study.professional.totalMinutes", "总时长", { parts: ["study.professional.corporateFinance.duration", "study.professional.investments.duration"] }), duration("study.professional.corporateFinance.duration", "公司金融"), duration("study.professional.investments.duration", "投资学"), text("study.professional.progress", "专业课今日推进"), text("study.professional.corporateFinance.progress", "公司金融推进"), text("study.professional.investments.progress", "投资学推进"), text("study.professional.adjustment", "调整"), text("study.professional.corporateFinance.adjustment", "公司金融调整"), text("study.professional.investments.adjustment", "投资学调整")] },
    { title: "英语", fields: [duration("study.english.totalMinutes", "总时长", { parts: ["study.english.vocabulary.duration", "study.english.ieltsWriting.duration", "study.english.ieltsReading.duration", "study.english.ieltsListening.duration", "study.english.ieltsSpeaking.duration"] }), duration("study.english.vocabulary.duration", "单词"), duration("study.english.ieltsWriting.duration", "雅思写作"), duration("study.english.ieltsReading.duration", "雅思阅读"), duration("study.english.ieltsListening.duration", "雅思听力"), duration("study.english.ieltsSpeaking.duration", "雅思口语"), text("study.english.progress", "英语今日推进"), text("study.english.vocabulary.progress", "单词推进"), text("study.english.ieltsWriting.progress", "雅思写作推进"), text("study.english.ieltsReading.progress", "雅思阅读推进"), text("study.english.ieltsListening.progress", "雅思听力推进"), text("study.english.ieltsSpeaking.progress", "雅思口语推进"), text("study.english.adjustment", "调整"), text("study.english.vocabulary.adjustment", "单词调整"), text("study.english.ieltsWriting.adjustment", "雅思写作调整"), text("study.english.ieltsReading.adjustment", "雅思阅读调整"), text("study.english.ieltsListening.adjustment", "雅思听力调整"), text("study.english.ieltsSpeaking.adjustment", "雅思口语调整")] },
...
export const otherSections = [
  { title: "睡眠", fields: [time("sleep.yesterday.bedtime", "入睡时间"), time("sleep.yesterday.wakeTime", "起床时间"), duration("sleep.yesterday.durationMinutes", "睡眠分钟数"), text("sleep.yesterday.durationText", "睡眠时长"), text("sleep.yesterday.lateReason", "晚睡原因"), text("sleep.yesterday.feeling", "睡眠感受"), text("sleep.yesterday.adjustment", "调整")] },
  ...
  { title: "个护", fields: [select("selfcare.today.basicSkincare", "基础护肤", ["是", "否"]), select("selfcare.today.mask", "面膜", ["否/未确认", "是"]), duration("selfcare.today.waterMl", "喝水量"), select("selfcare.today.period", "经期", ["否", "是"]), duration("selfcare.today.periodDay", "经期第几天"), select("selfcare.today.periodFlow", "血量", ["少量", "中等", "较多", "很多"]), select("selfcare.today.periodPain", "疼痛程度", ["无", "轻微", "中等", "明显", "严重"]), text("selfcare.today.other", "其他")] },
  { title: "状态", fields: [score("state.today.energy", "精力"), score("state.today.mood", "情绪"), score("state.today.body", "身体状态"), select("state.today.moodTag", "情绪", ["开心", "平静", "放松", "期待", "焦虑", "烦躁", "低落", "麻木", "复杂"]), select("state.today.bodyCondition", "身体状态", ["很好", "正常", "疲惫", "乏力", "不舒服", "疼痛", "生病"]), select("state.today.sleepImpact", "睡眠影响", ["大", "中", "小", "无"]), select("state.today.phoneInterference", "手机干扰", ["大", "中", "小", "无"]) ] },
...
// draft.ui holds per-day, draft-scoped UI state that must never leak across
// dates and must never live in profile: which specific study leaves the
// user added/removed for THIS day only. Contrast with
// profile.dailyReviewUi.defaultStudyLeaves, which is the cross-date "always
// show" list — adding a leaf for today never touches that.
// categoryVisibility mirrors studyLeafVisibility's shape/semantics but for
// dynamic (taxonomy-only, no static schema field) leaves under project/work/
// hobby/entertainment/family/misc/study — kept as a separate key so already-
// saved drafts' studyLeafVisibility never changes shape.
function defaultDraftUi() { return { studyLeafVisibility: { added: [], hidden: [] }, categoryVisibility: { added: [], hidden: [] } }; }
// draft.categoryReviewEntries holds today's values for taxonomy leaves that
// have NO static schema field (see src/review/reviewTaxonomyModel.js) —
// keyed by categoryId, each entry reusing the same {value, autoValue, source,
// manuallyEdited} field-state shape as draft.fields, just for
// duration/progress/adjustment sub-fields instead of a fixed field id.
function defaultCategoryReviewEntries() { return {}; }
export function createReviewDraft(date, profile = {}) { const fields = { "selfcare.today.basicSkincare": fieldState("是"), "selfcare.today.mask": fieldState("否/未确认"), "snowDust.note": fieldState("") }; [...allGroups(profile).flatMap((section) => section.groups), ...otherSections].forEach((group) => group.fields.forEach((field) => { if (!fields[field.id]) fields[field.id] = fieldState(""); })); return { schemaVersion: REVIEW_SCHEMA_VERSION, date, timezone: "Asia/Shanghai", status: "not_generated", fields, temporaryProjects: [], ui: defaultDraftUi(), categoryReviewEntries: defaultCategoryReviewEntries(), generatedAt: "", updatedAt: new Date().toISOString(), submittedAt: "", linkedSettlementId: "", revisionLog: [], clientRevision: 0 }; }
