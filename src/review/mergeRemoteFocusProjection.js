...
//   - the `autoValue` of fields/categoryReviewEntries the LAST TWO Focus
//     syncs targeted (current + previous, so a category that dropped out of
//     today's projection gets its old autoValue cleared too, not just left
//     stale)
//
// Always preserves local: `value`, `manuallyEdited`, `source`, any other key
// on each field/entry, draft.ui, clientRevision, and every field this
// mechanism never targets. Never full-replaces the draft.
...
export function mergeRemoteFocusProjection(localDraft, remoteDraft, { previousFieldProjection } = {}) {
  ...
  for (const fieldId of fieldTargets) {
    ...
    if (localField.autoValue === remoteField.autoValue) continue;
    if (!fieldsChanged) { fields = { ...fields }; fieldsChanged = true; }
    fields[fieldId] = { ...localField, autoValue: remoteField.autoValue };
  }
  ...
  for (const target of categoryEntryTargets) {
    ...
    if (localEntry.autoValue === remoteEntry.autoValue) continue;
    if (!entriesChanged) { categoryReviewEntries = { ...categoryReviewEntries }; entriesChanged = true; }
    categoryReviewEntries[categoryId] = { ...categoryReviewEntries[categoryId], [field]: { ...localEntry, autoValue: remoteEntry.autoValue } };
  }
