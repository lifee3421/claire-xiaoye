...
export function buildPlannerRules({ draft = {}, settings = {} } = {}) {
  const rules = [
    ...
    { id: "exercise-shower", source: "system", text: `如果安排运动，运动后要留出洗澡；当前洗澡预留 ${Math.max(0, Number(draft.showerMinutes || 0))} 分钟。` },
 ...
 * explicit proposals even when the planner page is closed.
 */
