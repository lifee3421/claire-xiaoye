 ...
 */
export function projectTrackerDailyOverview({ tracker = {}, facts = {}, today, hasSavedHistory = false, migrationState } = {}) {
  ...
  const historyNotMigrated = !hasCompletion && hasSavedHistory && (!migrationState || migrationState.status === "never_run");
  if (historyNotMigrated) return { kind: "history_not_migrated", status: "历史尚未迁移", lines: ["上次：历史尚未迁移"] };
  ...
  return {
    kind: "interval",
    status: hasCompletion ? statusLabel(facts.scheduleStatus) : "暂无完成记录",
    lines: [
      `上次：${hasCompletion ? dateWithRelativeLabel(facts.lastCompletedDate, today) : "暂无记录"}`,
